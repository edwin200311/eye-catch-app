...
function Main() {
  ...
  return (
    <div className="text-on-surface min-h-screen pb-28 bg-background font-body">
      <header className="bg-slate-50/70 backdrop-blur-xl fixed top-0 w-full z-50 shadow-sm flex justify-between items-center px-6 py-4">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-extrabold tracking-tighter text-blue-800 font-headline">Eye Catch</h1>
        </div>
        <div className="flex items-center gap-4">
          <span className="text-sm font-bold text-on-surface-variant">{userName} 님</span>
          <button onClick={handleLogout} className="text-xs bg-slate-200 px-3 py-1.5 rounded hover:bg-slate-300 transition-colors font-bold text-slate-700">로그아웃</button>
        </div>
      </header>
      ...
      <main className="pt-24 px-4 md:px-8 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          <div className="lg:col-span-8 space-y-6">
            ...
            <Link to="/incident-details" className="block bg-surface-container-lowest p-5 rounded-xl shadow-sm space-y-4 hover:shadow-md transition-shadow">
              ...
              <div className="flex items-center gap-4 bg-surface-container-low p-3 rounded-lg border-l-4 border-error">
                <div className="w-16 h-12 rounded bg-slate-200 overflow-hidden shrink-0 relative">
            ...
            <div className="bg-surface-container-lowest p-6 rounded-3xl shadow-sm border border-slate-100">
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between p-3 bg-blue-50 rounded-2xl">
                  <span className="text-xs font-semibold text-blue-800">활성 감지 구역</span>
                  <span className="text-lg font-headline font-extrabold text-blue-900">4</span>
                </div>
              </div>
            </div>
          </div>
      ...
      {/* 하단 네비게이션 */}
      <nav className="fixed bottom-0 left-0 w-full flex justify-around items-center px-4 pb-4 pt-2 bg-white shadow-[0_-4px_20px_rgba(0,0,0,0.05)] z-50 rounded-t-xl">
        <Link to="/main" className="flex flex-col items-center justify-center bg-blue-50 text-blue-800 rounded-2xl px-5 py-2 transition-all">
        ...
        <Link to="/zone" className="flex flex-col items-center justify-center text-slate-500 px-5 py-2 hover:text-blue-600 transition-all">
